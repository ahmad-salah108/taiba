...
@endpush
@section('content')
    <agent-trips-list :fetch-data-url="'{{route('agent.mange.trip.search')}}'"
                      :role="'admin'"
                      inline-template>
            ...
            <div class="card-body" style="overflow-x: scroll">
                ...
                >
                    ...
                    <table-column label="غرض الحجز" :sortable="false" :filterable="false">
                        <template scope="trip">
                            @{{trip.reservation_type.id==7?trip.reservation_type_text:trip.reservation_type.name}}
                    ...
                    <table-column label="طرف الحجز " :sortable="false" :filterable="false">
                        <template scope="trip">
                            <span v-if="trip.reserver">
                                      @{{ trip.reserver.id==38?trip.reserved_driver:trip.reserver.name }}
                            </span>
                        </template>
                    </table-column>
                    <table-column label="جهة التحصيل " :sortable="false" :filterable="false">
                        <template scope="trip">
                            <span v-if="trip.collector">
                                @{{ trip.collector.id==38?trip.collector_driver:trip.collector.name }}
                    ...
                    <table-column label="الاجراءات" :sortable="false" :filterable="false">
                        <template scope="user">
                            ...
                            <a class="btn btn-sm btn-clean btn-icon btn-icon-md"
                               :href="'/agent/trips/edit/'+user.id">
                                <i class="fa fa-pen" aria-hidden="true"></i>
                            </a>

                            <a class="btn btn-sm btn-clean btn-icon btn-icon-md"
                               href="javascript:void(0)" @click="duplicate(user.id)">
                                <i class="fa fa-copy" aria-hidden="true"></i>
                            </a>

                            <a class="btn btn-sm btn-clean btn-icon btn-icon-md"
                               @click="deleteAction('/agent/trips/destroy/',user.id,true)" href="javascript:;">
                ...
                <div class="row">
                    <div class="col-8">
                        <table class="table  table-striped table-hover ">
                            <thead>
                            <tr>
                                <th><strong>البيانات الرئيسية</strong></th>
                            </tr>
                            </thead>
                            <tbody>
                            <tr>
